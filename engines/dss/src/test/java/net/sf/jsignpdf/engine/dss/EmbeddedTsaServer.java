...
/**
 * Embedded RFC 3161 TSA server for the DSS engine tests, ported from the {@code jsignpdf-pades}
 * test suite. It runs in-JVM on a random loopback port (no external network), using a self-signed
 * timestamping certificate generated on the fly and BouncyCastle TSP to produce timestamp responses,
 * so level-T signatures can be produced and asserted deterministically.
 */
final class EmbeddedTsaServer {
     ...
     */
    void requireBasicAuth(String username, String password) {
        this.requiredUsername = username;
        this.requiredPassword = password;
    }

    /**
     * Generates a self-signed RSA key pair and certificate suitable for timestamping, and starts an HTTP
     * server on a random available loopback port.
     */
    void start() throws Exception {
        ...
        // Self-signed certificate with the id-kp-timeStamping extended key usage.
    ...
    /** @return the URL of the running TSA server. */
    String getUrl() {
    ...
    /**
     * @return the self-signed TSA certificate, so tests can pin it as a trust anchor. LTA adds an archive
     *         timestamp whose TSA chain must validate; trusting this (self-signed) certificate satisfies that
     *         without needing revocation data for the timestamp's own chain.
     */
    ...
    private final class TsaHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange exchange) throws IOException {
            try {
                ...
                DigestCalculator digestCalculator = new JcaDigestCalculatorProviderBuilder().setProvider("BC").build()
                        .get(new AlgorithmIdentifier(new ASN1ObjectIdentifier("1.3.14.3.2.26")));
