...
The author of the JSignPdf is Czech developer Josef (kwart) Cacek. He works in Java since 2000. Some links to Josef's projects:
...
Next four inputs __Lower Left (X, Y) __and _Upper Right (X, Y)_ define the position of the signature on the page. You can fill in float numbers (with decimal places) as input.
...
The classic Swing GUI is the original graphical interface from earlier versions of JSignPdf. It remains available for backward compatibility.

To start the Swing GUI, set the `jsignpdf.swing` system property:
...
All options available in the Swing GUI correspond to the options described in the <<Signing options>> chapter. Refer to those sections for detailed descriptions.
...
otherwise you can use JSignPdf in command line batch mode.
 ...
 -kst,--keystore-type <type>             sets KeyStore type (you can list
                                         ...
                                         argument)
 -l,--location <location>                location of a signature (e.g. Washington
                                         DC). Empty by default.
    --gui                                (beta) show GUI even when other command
                                         line arguments are provided
    --l2-text <text>                     signature text, you can also use
                                         placeholders for signature properties
                                         (${signer}, ${certificate},
                                         ${timestamp}, ${location}, ${reason},
                                         ${contact})
    --l4-text <text>                     status text
 -lk,--list-keys                         lists keys in chosen keystore
 -lkt,--list-keystore-types              lists keystore types, which can be used
                                         as values -kst option
 -llx <position>                         lower left corner position on X-axis of
                                         a visible signature (negative value is
                                         subtracted from page width)
 -lly <position>                         lower left corner position on Y-axis of
                                         a visible signature (negative value is
                                         subtracted from page height)
 ...
 -pe,--encryption <mode>                 encryption mode for the output PDF.
                                         ...
                                         together with -upwd and -opwd in case
 ...
 -sn,--signer-name <signer>              signer name. Defaults to the common name
                                         (CN) of the chosen certificate.
 ...
 -upwd,--user-password <password>        user password for encrypted documents
                                         (used when -e option is given)
 -urx <position>                         upper right corner position on X-axis of
                                         a visible signature (negative value is
                                         subtracted from page width)
 -ury <position>                         upper right corner position on Y-axis of
                                         a visible signature (negative value is
                                         subtracted from page height)
