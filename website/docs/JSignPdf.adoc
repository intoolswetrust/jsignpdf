...
Report bugs and request features in the issue tracker ({url-repo}/issues). Please don't use it for usage questions -- the Google Group is the place for those. Security vulnerabilities must not be reported as a public issue -- use {url-repo}/security/advisories/new[GitHub's private vulnerability reporting] instead, as described in the {url-repo}/blob/master/SECURITY.md[security policy].
