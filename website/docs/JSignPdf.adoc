...
Every run logs the certificate material it uses and, for the DSS engine, every network call it makes while collecting validation data. This is what to look at when a signature fails for a reason that is not obvious from the error message itself.
...
These messages are logged at the `FINE` level. To silence them (together with the rest of the progress output) use `-q` on the command line; to change what is logged, point Java at your own logging configuration with `JAVA_OPTS=-Djava.util.logging.config.file=<file>`.
