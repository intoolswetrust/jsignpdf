...
If you use the platform-independent ZIP archive, you need Java Runtime Environment (JRE) version 21 or newer. You can download it freely from the web, for instance:
...
Open _Tools > Preferences..._ (shortcut `Ctrl+,` / `⌘,`) for application-wide tweaks that fall outside the per-document signing options. The dialog has six tabs:
...
. Place a configuration file at `<config-dir>/pkcs11.cfg` (see <<Configuration directory>>). It is used to configure a Java SunPKCS11 security provider (see https://docs.oracle.com/en/java/javase/11/security/pkcs11-reference-guide1.html[PKCS#11 Reference Guide]). The easiest way to create or edit it is via _Tools > Preferences > PKCS#11_ in the JavaFX UI. +
...
On the first launch of 3.0.0, JSignPdf checks the install-dir copies of both files and migrates them only if you actually edited them (i.e. their contents differ from the bundled samples). Unmodified files are skipped to avoid carrying over stale example values like `library=/usr/lib/libeToken.so` into a fresh install. The original install-dir files are left in place so a downgrade keeps working.

=== Configuration directory

Starting with JSignPdf 3.0.0, per-user state is stored in a platform-native configuration directory instead of a single file in your home folder. The directory holds the main config file (`config.properties`, which persists the last-used signing settings), a `presets/` subdirectory (see <<Using presets>>), and -- once you visit the Preferences dialog or migrate from a 2.x install -- the advanced-config files `advanced.properties` and `pkcs11.cfg` (see <<Advanced application configuration>>).
...
Previous versions of JSignPdf stored everything in `~/.JSignPdf` (a single file in your home directory) plus the install-dir files `conf/conf.properties` and `conf/pkcs11.cfg`. On the first run of JSignPdf 3.0.0 or newer, if the new configuration directory does not yet exist, JSignPdf populates it from these locations: `~/.JSignPdf` is copied to `<config-dir>/config.properties`, and any edited install-dir files are copied to `<config-dir>/advanced.properties` and `<config-dir>/pkcs11.cfg` (see <<Advanced application configuration>>). An empty `presets/` subdirectory is created at the same time. **The legacy files are left in place**, so if you downgrade to an older JSignPdf release, it continues to work with its old settings.
