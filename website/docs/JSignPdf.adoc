...
|`jsignpdf-${VERSION}-macos-x64.dmg` +
`jsignpdf-${VERSION}-macos-aarch64.dmg`
|macOS disk images for Intel and Apple Silicon. Currently unsigned -- expect a Gatekeeper warning on first launch.

|`jsignpdf-${VERSION}-macos-x64.zip` +
`jsignpdf-${VERSION}-macos-aarch64.zip`
|Portable macOS `.app` bundles for Intel and Apple Silicon.
|===
