...
* a platform-independent ZIP archive containing JAR files (works on any OS with Java installed)
* a Windows installer (`.exe`) that bundles a Java runtime — no separate Java installation is needed
...
NOTE: The Windows installer bundles its own JRE, so a separate Java installation is not required.
...
| Loads properties from the given file. The file can be created by copying the default property file `.JSignPdf` from the user's home directory.
|===
...
| TSA password (for `PASSWORD` authentication).
...
Add `-Xmx<size>` to your Java command. The following example allows Java to use 512 MB of heap:
