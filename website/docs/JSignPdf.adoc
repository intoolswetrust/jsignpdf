...
|`true`/`false` (default `false`). When on, also trust this platform's OS / JVM certificate stores: `cacerts` everywhere, plus `Windows-ROOT` + `Windows-MY` on Windows and `KeychainStore` on macOS. These are public-CA / OS roots, not eIDAS trusted-list anchors: they let `LT`/`LTA` embed validation data for commercially-issued (non-qualified) certificates but do not confer qualified status.
