...
Presets capture everything in the <<Signing options>> reference (keystore, signature properties, visible-signature options, TSA, encryption and rights, OCSP/CRL, proxy) **except passwords**. Passwords are never stored in a preset file; you enter them separately for each signing session. The currently loaded input and output file paths are also not part of a preset.
...
Choose _Presets > Save current as new preset..._ from the menu bar. Enter a name (up to 60 characters, any Unicode letters / digits / punctuation; no `/`, `\`, tabs, or other control characters) and confirm. The preset appears in the toolbar drop-down immediately.
...
There is no export/import; presets are plain Java properties files under the <<Configuration directory,configuration directory>>, so you can copy them manually between machines if you want. Passwords saved with _Store passwords_ in the main config use a machine-local encryption seed, which is another reason preset files (which exclude passwords) are the safer way to share configuration.
