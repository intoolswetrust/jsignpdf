...
There is no export/import; presets are plain Java properties files under the <<Configuration directory,configuration directory>>, so you can copy them manually between machines if you want. If _Store passwords_ was enabled when the preset was saved, the file contains the signing passwords encrypted with a machine-local seed — the encrypted values are only usable on the same user account and are silently ignored elsewhere, but the ciphertext itself is still present in the file. If you intend to share a preset (email, chat, cloud storage), turn _Store passwords_ off before saving it so the encrypted password keys are not written at all.
