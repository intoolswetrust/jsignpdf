...
- **macOS Apple Silicon** — `jsignpdf-<version>-macos-aarch64.dmg` (unsigned — expect a Gatekeeper prompt on first launch), or the matching `.zip`. Intel Macs: run the Apple Silicon build under Rosetta 2, or use `jsignpdf-<version>-full.zip` with a locally installed Java 21.
