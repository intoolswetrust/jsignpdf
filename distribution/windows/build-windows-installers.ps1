...
Produces two artifacts in distribution/target/upload/:
  jsignpdf-<version>-windows-x64.msi  - WiX-based MSI installer with license page
  jsignpdf-<version>-windows-x64.zip  - portable zip of the jpackage app-image

Prerequisites on the build machine:
  * Azul Zulu+FX 21 (or another JDK 21 bundle that ships the JavaFX modules)
    on PATH and pointed at by JAVA_HOME. The script passes
    --add-modules javafx.controls,javafx.fxml,javafx.swing both to jlink and to
    the bundled launchers, so the JavaFX modules MUST be discoverable on the
    JDK's module path.
...
Write-Host "JSignPdf jpackage build (Windows x64)"
Write-Host "  version   : $Version"
Write-Host "  appVersion: $appVersion"
Write-Host "  root      : $root"

# WiX 3.x is preinstalled on GitHub windows-2022 runners; fall back to
...
if (-not (Test-Path $appassembly)) {
    throw "Missing distribution staging at $appassembly`nRun 'mvn -B -DskipTests -pl distribution -am package' first."
}
...
Get-ChildItem -Path $upload -Filter 'jsignpdf-*-windows-x64.*' -File -ErrorAction SilentlyContinue |
    Remove-Item -Force

# Copy the runtime jars into the jpackage staging area, but skip every JavaFX
# jar — JFX is provided by the bundled Zulu+FX runtime as named modules, so
# putting any JFX jar on the classpath would create split-package conflicts
# with the module-graph copies.
Get-ChildItem -Path $appassembly -Filter '*.jar' -File |
    Where-Object { $_.Name -notlike 'javafx-*' } |
    ForEach-Object { Copy-Item $_.FullName (Join-Path $staging $_.Name) }

$mainJar = Get-ChildItem -Path $staging -Filter 'jsignpdf-*.jar' -File |
    Where-Object { $_.Name -notlike 'jsignpdf-bootstrap-*' -and $_.Name -notlike 'jsignpdf-jpedal*' } |
    Select-Object -First 1
if (-not $mainJar) {
    throw "Could not locate the jsignpdf main jar inside $staging."
}
$installCertJar = Get-ChildItem -Path $staging -Filter 'installcert-*.jar' -File | Select-Object -First 1
if (-not $installCertJar) {
    throw "Could not locate installcert-*.jar inside $staging."
}

Write-Host "  main jar  : $($mainJar.Name)"
Write-Host "  cert jar  : $($installCertJar.Name)"
...
# Bundle the demo folder (sample PDF + demo keystore + usage README).
...
# InstallCert inherit them; JSignPdf-swing gets a generated .properties file
# below that concatenates these options with '-Djsignpdf.swing=true'. We also
# always need --add-modules so JavaFX modules from the bundled runtime are
# readable to the unnamed module that holds our classpath classes.
...
$jfxAddModules = '--add-modules=javafx.controls,javafx.fxml,javafx.swing'
if (-not ($commonJvmOptions -contains $jfxAddModules)) {
    $commonJvmOptions = @($jfxAddModules) + $commonJvmOptions
}
...
$zipPath = Join-Path $upload "jsignpdf-$Version-windows-x64.zip"
Write-Host "==> Compress-Archive -> $zipPath"
if (Test-Path $zipPath) { Remove-Item -Force $zipPath }
Compress-Archive -Path $appImage -DestinationPath $zipPath

# 3. Build the MSI installer from the app-image. The EXE flavour from earlier
# versions is dropped — MSI provides the same double-click UX plus enterprise
# deployability, and SignPath signing targets MSI.
$installerArgs = @(
    '--app-image',$appImage,
    '--name','JSignPdf',
    '--app-version',$appVersion,
...
Move-Item (Join-Path $out "JSignPdf-$appVersion.msi") (Join-Path $upload "jsignpdf-$Version-windows-x64.msi") -Force
