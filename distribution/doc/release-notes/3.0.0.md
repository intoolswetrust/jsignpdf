...
- **Configuration has moved** to a platform-appropriate XDG-aware
  ...
  run. Override with `JSIGNPDF_CONFIG_DIR` for portable installs. The
  install-dir `conf/` directory (`conf.properties`, `pkcs11.cfg`) is no
  longer shipped — advanced settings are edited via the new
  **File > Preferences…** dialog and stored under the per-user config
  directory as `advanced.properties` / `pkcs11.cfg`. The 2.x
  `pkcs11config.path` key is removed; PKCS#11 configuration is now
  read from a fixed well-known location, `<config-dir>/pkcs11.cfg`.
  Pre-3.0.0 install-dir tweaks are not migrated automatically — re-enter
  them via the Preferences dialog or copy the files by hand.
