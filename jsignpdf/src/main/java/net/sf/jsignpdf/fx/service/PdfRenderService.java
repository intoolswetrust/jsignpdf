 ...
 */
public class PdfRenderService extends Service<RenderedPage> {
    ...
    @Override
    protected Task<RenderedPage> createTask() {
        final BasicSignerOptions taskOptions = this.options;
        final int taskPage = this.page;

        return new Task<RenderedPage>() {
            @Override
            protected RenderedPage call() {
                ...
                return new RenderedPage(SwingFxImageConverter.toFxImage(buffered), p2i.getLastRenderDpi());
            }
        };
    }
}
