...
public class AcroFields {
  ...
  private Map sigNamesMap;
  private List sigNames;
  private boolean append;
  public static final int DA_FONT = 0;
  public static final int DA_SIZE = 1;
  public static final int DA_COLOR = 2;
  private Map extensionFonts = new HashMap();
  private XfaForm xfa;
  ...
  /**
   * Gets the list of export option values from fields of type list or combo. If
   * the field doesn't exist or the field type is not list or combo it will
   ...
   * <CODE>null</CODE> but not both. This method will only set the list but will
   * not set the value or appearance. For that, calling <CODE>setField()</CODE>
   * is required.
   * <p>
   * An example:
   * <p>
  ...
  Color getMKColor(PdfArray ar) {
    ...
    case 3:
      return new Color(ExtendedColor.normalize(ar.getAsNumber(0).floatValue()), ExtendedColor.normalize(ar.getAsNumber(
          1).floatValue()), ExtendedColor.normalize(ar.getAsNumber(2).floatValue()));
    case 4:
   ...
   * <li>clrflags - a set of flags to be cleared (turned off) in the F entry of
   * the form's corresponding widget annotation dictionary. Bits equal to 1
   * cause the corresponding bits in F to be set to 0.<br>
   * <li>fflags - a set of flags specifying various characteristics of the
   * field. The value of this entry replaces that of the Ff entry in the form's
   * corresponding field dictionary.<br>
   * <li>setfflags - a set of flags to be set (turned on) in the Ff entry of the
   * form's corresponding field dictionary. Bits equal to 1 cause the
   ...
   * 
   * @param name
   *          the fully qualified field name or the partial name in the case of
   *          XFA forms
   * @throws IOException
   *           on error
   * @throws DocumentException
  ...
  /**
   * Sets the field value and the display string. The display string is used to
   * build the appearance in the cases where the value is modified by Acrobat
   * with JavaScript and the algorithm is known.
   * 
   * @param name
   *          the fully qualified field name or the partial name in the case of
   *          XFA forms
  ...
  /**
   * Sets different values in a list selection. No appearance is generated yet;
   * nor does the code check if multiple select is allowed.
   * 
   * @param name
  ...
  /**
   * Gets all the fields. The fields are keyed by the fully qualified field name
   * and the value is an instance of <CODE>AcroFields.Item</CODE>.
   * 
    ...
    for (int k = 0; k < item.size(); ++k) {
      try {
        ...
        if (rotation != 0) {
          ...
          switch (rotation) {
          case 270:
            r = new Rectangle(pageSize.getTop() - r.getBottom(), r.getLeft(), pageSize.getTop() - r.getTop(),
                r.getRight());
            break;
          case 180:
            r = new Rectangle(pageSize.getRight() - r.getLeft(), pageSize.getTop() - r.getBottom(), pageSize.getRight()
                - r.getRight(), pageSize.getTop() - r.getTop());
            break;
          case 90:
            r = new Rectangle(r.getBottom(), pageSize.getRight() - r.getLeft(), r.getTop(), pageSize.getRight()
                - r.getRight());
            break;
          }
          r.normalize();
   ...
   *          the field name
   * @param page
   *          the page to remove the field from or -1 to remove it from all the
   *          pages
  ...
  public static class Item {
     ...
     * merged, widget, and/or value, depending on the <code>writeFlags</code>
     * setting
     * 
     * @since 2.1.5
     * 
    ...
    /**
     * An array of <CODE>PdfDictionary</CODE> with all the field and widget tags
     * merged.
     * 
     * @deprecated (will remove 'public' in the future)
     */
    ...
    /**
     * An array of <CODE>Integer</CODE> with the page numbers where the widgets
     * are displayed.
     * 
     * @deprecated (will remove 'public' in the future)
     */
    ...
    /**
     * An array of <CODE>Integer</CODE> with the tab order of the field in the
     * page.
     * 
     * @deprecated (will remove 'public' in the future)
     */
    ...
    /**
     * Remove the given instance from this item. It is possible to remove all
     ...
     * @param idx
     *          instance index
     * @return dictionary storing this instance's value. It may be shared across
     *         instances.
     */
     ...
     * may have been overwritten (or modified?) by children. Example: a merged
     * radio field dict will contain /V
     * 
     * @since 2.1.5
     * @param idx
     *          instance index
  ...
  public ArrayList getBlankSignatureNames() {
    ...
    for (Iterator it = fields.entrySet().iterator(); it.hasNext();) {
      ...
      if (!PdfName.SIG.equals(merged.getAsName(PdfName.FT)))
        continue;
      if (sigNamesMap.containsKey(entry.getKey()))
        continue;
  ...
  public int getRevision(String field) {
    ...
    if (!sigNamesMap.containsKey(field))
      return 0;
    return ((int[]) sigNamesMap.get(field))[1];
  }
  ...
  public InputStream extractRevision(String field) throws IOException {
    ...
    if (!sigNamesMap.containsKey(field))
      return null;
    int length = ((int[]) sigNamesMap.get(field))[0];
   ...
   * <CODE>BaseFont</CODE> and can be <CODE>null</CODE>. The fonts in this list
   ...
   * <CODE>BaseFont</CODE> and can also be <CODE>null</CODE>. The fonts in this
   ...
   * pushbuttons with the same name only the first one is used. This pushbutton
   * can be changed and be used to replace an existing one, with the same name
   * or other name, as long is it is in the same document. To replace an
   * existing pushbutton call
   ...
   * changed and be used to replace an existing one, with the same name or other
   * name, as long is it is in the same document. To replace an existing
   ...
   * @return <CODE>true</CODE> if the field was replaced, <CODE>false</CODE> if
   *         the field was not a pushbutton
   */
  ...
  /**
   * Replaces the designated field with a new pushbutton. The pushbutton can be
   * created with {@link #getNewPushbuttonFromField(String,int)} from the same
   * document or it can be a generic PdfFormField of the type pushbutton.
   * 
   * @param field
   *          the field name
