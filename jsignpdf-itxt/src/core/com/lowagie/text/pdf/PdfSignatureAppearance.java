...
public class PdfSignatureAppearance {
	...
	/**
	 * Sets the rendering mode for this signature. The rendering modes can be
	 * the constants <CODE>SignatureRenderDescription</CODE>,
	 ...
	 * 
	 * @param text
	 *            the signature text identifying the signer. If
	 *            <CODE>null</CODE> or not set a standard description will be
	 *            used
	 */
	 ...
	 * 
	 * @param text
	 *            the text identifying the signature status. If
	 *            <CODE>null</CODE> or not set the description
	 *            "Signature Not Verified" will be used
	 */
	 ...
	 * @param filter
	 *            the crytographic filter type. It can be SELF_SIGNED,
	 ...
	 * @param fieldName
	 *            the field name or <CODE>null</CODE> to generate automatically
	 *            a new field name
	 */
	...
	public void setVisibleSignature(String fieldName) {
		...
		switch (rotation) {
		case 90:
			pageRect = new Rectangle(pageRect.getBottom(), pageSize.getTop() - pageRect.getLeft(), pageRect.getTop(),
	 ...
	 * <p>
	 * Consult <A HREF=
	 * "http://partners.adobe.com/asn/developer/pdfs/tn/PPKAppearances.pdf"
		...
		if (app[2] == null) {
			...
			if (image != null) {
				if (imageScale == 0) {
					...
					if (imageScale < 0)
						usableScale = Math
								.min(rect.getWidth() / image.getWidth(), rect.getHeight() / image.getHeight());
			...
			if (render == SignatureRenderNameAndDescription
				...
				dataRect = new Rectangle(MARGIN, MARGIN, rect.getWidth() - MARGIN, rect.getHeight() * (1 - TOP_SECTION)
						- MARGIN);
			}
	 ...
	 * 
	 * @return <CODE>true</CODE> if a new field was created, <CODE>false</CODE>
	 ...
	 * 
	 * @return the temporary file or <CODE>null</CODE> is the document is
	 *         created in memory
	 */
	 ...
	 * If calling preClose() <B>dont't</B> call PdfStamper.close().
	 * <p>
	 * If using an external signature <CODE>exclusionSizes</CODE> must contain
	 * at least the <CODE>PdfName.CONTENTS</CODE> key with the size that it will
	 * take in the document. Note that due to the hex string coding this size
	 ...
	 * general sequence is: preClose(), getDocumentBytes() and close().
	 * <p>
	 * <CODE>update</CODE> is a <CODE>PdfDictionary</CODE> that must have
	 * exactly the same keys as the ones provided in {@link #preClose(HashMap)}.
	 * 
	 * @param update
	 *            a <CODE>PdfDictionary</CODE> with the key/value that will fill
	 *            the holes defined in {@link #preClose(HashMap)}
	 * @throws DocumentException
	 *             on error
	 * @throws IOException
	 *             on error
			...
			for (Iterator it = update.getKeys().iterator(); it.hasNext();) {
				...
				if (lit == null)
					throw new IllegalArgumentException("The key " + key.toString()
							+ " didn't reserve space in preClose().");
				bf.reset();
				obj.toPdf(null, bf);
				if (bf.size() > lit.getPosLength())
					throw new IllegalArgumentException("The key " + key.toString() + " is too big. Is " + bf.size()
							+ ", reserved " + lit.getPosLength());
				if (tempFile == null)
	...
	/**
	 * Sets the signature event to allow modification of the signature
	 * dictionary.
	 * 
