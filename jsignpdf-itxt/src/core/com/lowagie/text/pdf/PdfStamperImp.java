...
class PdfStamperImp extends PdfWriter {
	 ...
	 * @throws IOException
	 */
	PdfStamperImp(PdfReader reader, OutputStream os, char pdfVersion, boolean append) throws DocumentException,
			IOException {
	...
	void close(HashMap moreInfo) throws IOException {
		...
		markUsed(pages);
		PdfDictionary acroForm = (PdfDictionary) PdfReader.getPdfObject(catalog.get(PdfName.ACROFORM), reader
				.getCatalog());
					...
					if (obj != null) {
						...
						if (obj instanceof PdfIndirectReference && !obj.isIndirect())
							...
							if (objReal != null && objReal.isDictionary()) {
								...
								if (as != null) {
									PdfIndirectReference iref = (PdfIndirectReference) ((PdfDictionary) objReal)
											.get(as);
									if (iref != null) {
		...
		for (int page = 1; page <= reader.getNumberOfPages(); ++page) {
			...
			for (int idx = 0; idx < annots.size(); ++idx) {
				...
				if ((flags & PdfFormField.FLAGS_PRINT) != 0 && (flags & PdfFormField.FLAGS_HIDDEN) == 0) {
					...
					if (obj1 == null)
						continue;
					PdfDictionary appDic = (obj1 instanceof PdfIndirectReference) ? (PdfDictionary) PdfReader
							.getPdfObject(obj1) : (PdfDictionary) obj1;
					PdfObject obj = appDic.get(PdfName.N);
					PdfAppearance app = null;
					PdfObject objReal = PdfReader.getPdfObject(obj);
	...
	void addFieldResources() throws IOException {
		...
		// if (dr.get(PdfName.ENCODING) == null) dr.put(PdfName.ENCODING,
		// PdfName.WIN_ANSI_ENCODING);
				...
				if (annot.isAnnotation()) {
					...
					if (!annot.isUsed()) {
						...
						if (rect != null
							...
							switch (rotation) {
							case 90:
								annot.put(PdfName.RECT, new PdfRectangle(pageSize.getTop() - rect.bottom(),
										rect.left(), pageSize.getTop() - rect.top(), rect.right()));
								break;
							case 180:
								annot.put(PdfName.RECT, new PdfRectangle(pageSize.getRight() - rect.left(), pageSize
										.getTop()
										- rect.bottom(), pageSize.getRight() - rect.right(), pageSize.getTop()
										- rect.top()));
								break;
							case 270:
								annot.put(PdfName.RECT, new PdfRectangle(rect.bottom(), pageSize.getRight()
										- rect.left(), rect.top(), pageSize.getRight() - rect.right()));
								break;
							}
						}
					}
	...
	void addFileAttachments() throws IOException {
		...
		markUsed(names);
		HashMap old = PdfNameTree.readTree((PdfDictionary) PdfReader.getPdfObjectRelease(names
				.get(PdfName.EMBEDDEDFILES)));
	...
	/**
	 * Additional-actions defining the actions to be taken in response to
	 * various trigger events affecting the document as a whole. The actions
	 * types allowed are: <CODE>DOCUMENT_CLOSE</CODE>, <CODE>WILL_SAVE</CODE>,
	 * <CODE>DID_SAVE</CODE>, <CODE>WILL_PRINT</CODE> and <CODE>DID_PRINT</CODE>
	 * .
	 * 
	 * @param actionType
	 *            the action type
	...
	public void setAdditionalAction(PdfName actionType, PdfAction action) throws PdfException {
		if (!(actionType.equals(DOCUMENT_CLOSE) || actionType.equals(WILL_SAVE) || actionType.equals(DID_SAVE)
				|| actionType.equals(WILL_PRINT) || actionType.equals(DID_PRINT))) {
	...
	/**
	 * Reads the OCProperties dictionary from the catalog of the existing
	 * document and fills the documentOCG, documentOCGorder and OCGRadioGroup
	 * variables in PdfWriter. Note that the original OCProperties of the
	 * existing document can contain more information.
	 * 
	 * @since 2.1.2
	 */
	...
	/**
	 * Recursive method to reconstruct the documentOCGorder variable in the
	 * writer.
	 * 
	 * @param parent
	 *            a parent PdfLayer (can be null)
	 * @param arr
	 *            an array possibly containing children for the parent PdfLayer
	 * @param ocgmap
	 *            a HashMap with indirect reference Strings as keys and PdfLayer
	 *            objects as values.
	 * @since 2.1.2
	 */
