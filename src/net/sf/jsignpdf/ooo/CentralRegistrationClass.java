...
public class CentralRegistrationClass {
		...
		while (t.hasMoreTokens()) {
			...
			if (className != null && className.length() != 0) {
				try {
					...
					Method writeRegInfo = regClass.getDeclaredMethod(
							"__getComponentFactory",
							new Class[] { String.class });
					Object result = writeRegInfo.invoke(regClass,
							sImplementationName);
					if (result != null) {
	...
	private static String getRegistrationClasses() {
		...
		String name = c.getClass().getCanonicalName().replace('.', '/').concat(
		".class");
		try {
			Enumeration<URL> urlEnum = c.getClass().getClassLoader()
