...
public class PKCS11Utils {
	...
	public static boolean registerProvider(final String configPath) {
		...
		if (cfgFile.isFile()) {
			try {
				Security.addProvider((Provider) Class.forName("sun.security.pkcs11.SunPKCS11").getConstructor(
						String.class).newInstance(absolutePath));
				return true;
			} catch (Exception e) {
				System.err
						.println("Unable to register SunPKCS11 security provider. The original exception message is:\n"
								+ e.getMessage());
			}
		} else {
