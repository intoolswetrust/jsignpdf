...
| macOS x64                      | DMG          | yes         | yes (in JDK)           | Intel Macs (best-eff.)|
| macOS x64                      | ZIP          | yes         | yes (in JDK)           | Portable (best-eff.)  |
...
package              (matrix, needs: build-jars)
  ...
  ├─ macos-13          → DMG + macOS x64 ZIP        (unsigned; BEST-EFFORT)
...
- **`fail-fast: false`** so one platform failure does not abort the others
  mid-release.
- **Intel macOS (`macos-13`) is best-effort.** GitHub's last Intel macOS
  image has a heavily constrained, being-retired hosted pool — jobs routinely
  sit queued for hours or never get a runner, while arm64 (`macos-14`)
  finishes in minutes. The `macos-x64` matrix entry sets
  `continue-on-error`, and the `package` job has `timeout-minutes: 60`, so a
  stuck/failed Intel build neither hangs nor blocks the release; the GitHub
  Release simply ships without the `macos-x64` DMG/ZIP that run. `jpackage`
  only builds for the host arch, so an x64 DMG requires an Intel host and
  there is no newer Intel runner to migrate to.
