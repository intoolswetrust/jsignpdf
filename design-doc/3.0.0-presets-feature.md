...
- Passwords (`ksPasswd`, `keyPasswd`, `pdfOwnerPwd`, `pdfUserPwd`, `tsaPasswd`, `tsaCertFilePwd`). Rationale: encrypted-with-local-seed values are useless if copied off-machine, and keeping them in presets invites accidental credential leakage when users share files. Loading a preset leaves the live password fields untouched (whatever the user last entered, or empty).
...
```
Presets
 ├─ Save current as new preset…   (Ctrl+Shift+S)
 └─ Manage presets…
```
