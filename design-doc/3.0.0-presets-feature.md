...
### 9.1 Combo box in the main window

A new row at the top of `MainWindow.fxml`, above the accordion:

```
Preset: [ Signed with TSA and stamp ▾ ]  [Save]  [Save as…]  [Delete]  [Manage…]
```

- Combo contents come straight from `PresetManager.getPresets()` (ObservableList). Sorted by display name.
- Selecting a preset: `PresetManager.load(preset)` → `signingVM.syncFromOptions(opts)`. Unsaved changes prompt a confirmation first.
- A "*" suffix appears on the selected label when `dirty` is true.
- When no preset is active, the combo shows `<unsaved>` as a disabled-looking placeholder.
- Buttons:
  - **Save** — overwrite the active preset. Disabled when no preset is active; becomes "Save as…" implicitly in that case.
  - **Save as…** — dialog asks for display name, creates a new preset, selects it.
  - **Delete** — confirmation, then removes file and clears selection.
  - **Manage…** — opens a small dialog listing all presets with per-row Rename / Delete. No export/import.
...
Presets
 ...
 ├─ Load                    ▸  ├─ Signed with TSA and stamp
 │                             ├─ Signed without TSA
 │                             └─ Draft (no TSA)
 ├─ ───────
...
The Load submenu rebuilds from `PresetManager.getPresets()` on menu open. Ctrl+S on an unsaved-settings app with no active preset opens Save as.
...
- Reject `/` and `\` (users typing a path by mistake).
- Length cap at 100 chars — UI sanity, not a filesystem limit.
...
- `jfx.gui.preset.button.save`, `.saveAs`, `.delete`, `.manage`
- `jfx.gui.menu.presets`, `jfx.gui.menu.presets.save`, `.saveAs`, `.load`, `.delete`, `.manage`
...
- Combo + menu parity: every operation available in both.
- Dirty-flag tracking across every field edit.
- "Discard unsaved changes" prompt appears at the right moments.
